...
export default function OverviewPage() {
  ...
  if (loading) return <div className="page-state">Loading overview…</div>;
    ...
    <div className="page-stack">
      <section className="hero-panel">
        <div className="hero-panel__copy">
          <div className="hero-panel__label">Political attention, made explorable</div>
          ...
          <div className="hero-panel__actions">
            <Link className="button button--primary" to="/topics">Open Topic Atlas</Link>
            <Link className="button button--secondary" to="/speeches">Browse Speeches</Link>
          </div>
        </div>
      ...
      <section className="stats-strip">
        <div className="stat-block"><span>{formatNumber(summary.speechesAnalyzed)}</span><label>Speeches analyzed</label></div>
        <div className="stat-block"><span>{summary.macroTopicCount}</span><label>Macro-topics</label></div>
        <div className="stat-block"><span>{summary.yearsCovered[0]}–{summary.yearsCovered[1]}</span><label>Years covered</label></div>
        <div className="stat-block"><span>{Object.keys(summary.languages).join(' · ')}</span><label>Languages</label></div>
      </section>
      ...
      <section className="story-grid">
        <div className="story-grid__main editorial-panel">
          ...
          <D3TimelineChart temporalData={temporal} selectedTopicKeys={summary.topTopics.map((topic) => String(topic.topicId)).slice(0, 4)} onToggleTopic={(topicId) => navigate(`/timeline?topic=${topicId}`)} />
        </div>
